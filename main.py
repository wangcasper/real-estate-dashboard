...
if st.session_state.active_page == "Statistics":
    ...
    if 'df_clsd' in st.session_state:
        ...
        # ---- 📌 Key Summary Metrics (Past 12 Months) ----
        st.markdown("---")
        st.subheader("📌 Key Summary Metrics (Last 12 Months)")

        ed_date = pd.Timestamp(st.session_state.ed_date)
        start_date = ed_date - pd.DateOffset(months=12)
        st.write(f"Date Range:   **{start_date.date()}** to **{ed_date.date()}**")

        df_last12 = df_filtered[
            (df_filtered['Closed_Date'] >= start_date) &
            (df_filtered['Closed_Date'] <= ed_date)
        ]

        recent_median_price = df_last12['Sold_Price'].median()
        recent_median_days = df_last12['Market_Time'].median()
        recent_total = len(df_last12)

        col1, col2, col3 = st.columns(3)
        col1.metric("12-Month Median Price", f"${int(recent_median_price):,}" if pd.notna(recent_median_price) else "N/A")
        col2.metric("12-Month Median Days", f"{int(recent_median_days)} days" if pd.notna(recent_median_days) else "N/A")
        col3.metric("12-Month Total Properties", recent_total)

        # ---- 📅 0–12 Month Summary ----
        st.markdown("---")
        st.subheader("🕐 0–12 Month Summary")
        summary_text = generate_12_month_summary(df_last12)
...
# Quarterly Analysis
    ...
    if 'df_filtered' in st.session_state:
        ...
        quarter_labels = [f"Q{i+1}" for i in range(20)]
        quarter_ranges = []
        for i in range(20):
        ...
        # Keep default order as Q1 → Q20 (newest to oldest)
        quarter_labels = [q["Quarter"] for q in quarter_ranges]

        # Let user select which Qs to display (default show all, newest to oldest)
        q_start, q_end = st.select_slider(
            'Select Quarter Range to Display',
            options=quarter_labels,
            value=(quarter_labels[0], quarter_labels[-1])  # default: all
        )
        ...
        if summary.empty:
            ...
            # ✅ Sort by Q index: Q1, Q2, ..., newest to oldest
            summary["Q_Index"] = summary["Quarter"].str.extract(r'Q(\d+)').astype(int)
            summary = summary.sort_values(by="Q_Index").drop(columns="Q_Index")

            chart_type = st.selectbox("Select Chart Type", ["line", "scatter", "histogram"])
            plot_chart(summary, "Quarter", chart_type, x_order=summary["Quarter"].tolist())

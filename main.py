...
# Yearly Analysis
    ...
    if 'df_filtered' in st.session_state:
        ...
        year_list = pd.date_range(start=start_ed, end=end_ed, freq='YS').year.astype(str).tolist()[::-1]
        ...
        start_label = year_list[0]
        end_label = year_list[-1]
        st.subheader(f"📊 Yearly Summary ({start_label} to {end_label})")
        plot_combo_chart_with_table(summary, x_col="Year", x_order=year_list)
...
# Quarterly Analysis
    ...
    if 'df_filtered' in st.session_state:
        ...
        q_start, q_end = st.select_slider(
            ...
            value=(quarter_labels[0], quarter_labels[7])  # default: all
        )
        ...
        if summary.empty:
            ...
            plot_combo_chart_with_table(summary, x_col="Quarter", x_order=summary["Quarter"].tolist())
...
# Monthly Analysis
    ...
    if 'df_filtered' in st.session_state:
        ...
        month_list = pd.period_range(start=start_ed, end=end_ed, freq='M').astype(str).tolist()[::-1]
